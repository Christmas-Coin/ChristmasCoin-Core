...
#if defined(NDEBUG)
#error "christmascoin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60*90; // 1.5 hours
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zchristmascoin spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zchristmascoin spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zchristmascoin spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 32270 && nHeight > 21841) {
		nSubsidy = 6 * COIN;
	}
	else if (nHeight <= 31680 && nHeight > 30270) {
		nSubsidy = 20 * COIN;
	}
	else if (nHeight <= 40100 && nHeight > 31680) {
		nSubsidy = 8 * COIN;
	}
	else if (nHeight <= 41500 && nHeight > 40100) {
		nSubsidy = 25 * COIN;
	}

	else if (nHeight < 51330 && nHeight > 41500) {
		nSubsidy = 10 * COIN;
	}

	else if (nHeight <= 55550 && nHeight > 51330) {
		nSubsidy = 60 * COIN;
	}

	else if (nHeight <= 61170  && nHeight > 55550) {
		nSubsidy = 12 * COIN;
	}

	else if (nHeight <= 62570 && nHeight > 61170) {
		nSubsidy = 30 * COIN;
	}

	else if (nHeight <= 71050 && nHeight > 62570) {
		nSubsidy = 14 * COIN;
	}

	else if (nHeight <= 72410 && nHeight > 71050) {
		nSubsidy = 60 * COIN;
	}

	else if (nHeight <= 80840 && nHeight > 72410) {
		nSubsidy = 15 * COIN;
	}

	else if (nHeight <= 90680 && nHeight > 80840) {
		nSubsidy = 10 * COIN;
	}

	else if (nHeight <= 100150 && nHeight > 90680) {
		nSubsidy = 8 * COIN;
	}

	else if (nHeight <= 110360 && nHeight > 100150) {
		nSubsidy = 6 * COIN;
	}

	else if (nHeight <= 526710 && nHeight > 110360) {
		nSubsidy = 0 * COIN;
	}

	else if (nHeight <= 613110 && nHeight > 526710) {
		nSubsidy = 15 * COIN;
	}

	else if (nHeight > 613110) {
		nSubsidy = 0.1 * COIN;
	}
...
ret = blockValue * 0.5;
	...
	else if (nHeight <= 30270 && nHeight > 21841) {
ret = blockValue * 0.7;
	}
	else if (nHeight <= 31680 && nHeight > 30270) {
ret = blockValue * 0.5;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from christmascoin
...
{
    RenameThread("christmascoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZCHMCMinted()
{
...
void RecalculateZCHMCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zCHMC supply
...
bool RecalculateCHMCSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zchristmascoin with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zchristmascoinSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CHMC to zCHMC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zchristmascoin spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zchristmascoin serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zchristmascoin mints and %d zchristmascoin spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // christmascoin: We use certain sporks during IBD, so check to see if they are
