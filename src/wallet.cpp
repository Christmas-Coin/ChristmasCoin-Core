...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 CHMC
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 CHMC over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 CHMC+1 ( bit on if present )
            // bit 1 - 1000 CHMC+1
            // bit 2 - 100 CHMC+1
            // bit 3 - 10 CHMC+1
            // bit 4 - 1 CHMC+1
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 CHMC.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 CHMC.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-christmascoin-address
                    CScript scriptChange;
...
void CWallet::AutoZeromint()
{
    ...
    // zCHMC are integers > 0, so we can't mint 10% of 9 CHMC
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zCHMC\n", nBalance);
        return;
    }

    // Percentage of zCHMC we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zCHMC (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zCHMC amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zCHMC amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zCHMC We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough CHMC to mint this denomination
    ...
    CWalletTx wtx;
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zCHMC. Current percentage of zCHMC: %lf%%\n",
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CZerocoinMint>& vMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to CHMC
    ...
    // Sign if these are christmascoin outputs - NOTE that zCHMC outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt)
{
    ...
    receipt.SetStatus("Transaction Mint Started", ZCHMC_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus("the selected mint coin is an invalid coin", ZCHMC_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason)) {
        receipt.SetStatus("Try to spend with a higher security level to include more coins", ZCHMC_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus("the new spend coin transaction did not verify", ZCHMC_INVALID_WITNESS);
            return false;
        }
        ...
        //Add the coin spend into a christmascoin transaction
        ...
        try {
            ...
            receipt.SetStatus("failed to deserialize", ZCHMC_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus("the transaction did not verify", ZCHMC_BAD_SERIALIZATION);
            return false;
        }
        ...
        for (const CBigNum& item : listCoinSpendSerial) {
            if (spend.getCoinSerialNumber() == item) {
                //Tried to spend an already spent zCHMC
                ...
                receipt.SetStatus("the coin spend has been used", ZCHMC_SPENT_USED_ZCHMC);
                return false;
            }
        }
        ...
        receipt.SetStatus("CoinSpend: Accumulator witness does not verify", ZCHMC_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus("Spend Valid", ZCHMC_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CZerocoinMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    int nStatus = ZCHMC_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus("Value is below the the smallest available denomination (= 1) of zchristmascoin", nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZCHMC_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zCHMC transaction
    if (vSelectedMints.empty()) {
        ...
        // Select the zchristmascoin mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to christmascoin address to the transaction (the actual primary spend taking place)
    ...
    receipt.SetStatus("Transaction Created", ZCHMC_SPEND_OKAY); // Everything okay
...
void CWallet::ZchristmascoinBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozchristmascoinbackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozchristmascoinbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozchristmascoinbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    int nStatus = ZCHMC_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZCHMC_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZCHMC_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& mint : vNewMints) {
            if (!walletdb.EraseZerocoinMint(mint)) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZCHMC_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    receipt.SetStatus("Spend Successful", ZCHMC_SPEND_OKAY);  // When we reach this point spending zCHMC was successful
