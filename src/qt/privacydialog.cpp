...
#include "zchristmascoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCHMC ought to be enough for anybody." - Bill Gates, 2017
    ui->zCHMCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCHMCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCHMC->setEnabled(false);
        ui->pushButtonMintzCHMC->setToolTip(tr("zCHMC is currently disabled due to maintenance."));

        ui->pushButtonSpendzCHMC->setEnabled(false);
        ui->pushButtonSpendzCHMC->setToolTip(tr("zCHMC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCHMCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCHMC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCHMC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCHMC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCHMC in ") +
...
void PrivacyDialog::on_pushButtonSpendzCHMC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCHMC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCHMC
        sendzCHMC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCHMC
    sendzCHMC();
}

void PrivacyDialog::on_pushButtonZchristmascoinControl_clicked()
{
    ZchristmascoinControlDialog* zchristmascoinControl = new ZchristmascoinControlDialog(this);
    zchristmascoinControl->setModel(walletModel);
    zchristmascoinControl->exec();
}

void PrivacyDialog::setZchristmascoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzchristmascoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCHMC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid christmascoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCHMCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCHMC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CHMC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCHMC</b>";
    ...
    // use mints from zchristmascoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZchristmascoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZchristmascoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zchristmascoin selector in case it was used
    ZchristmascoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zchristmascoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " christmascoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zchristmascoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCHMC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCHMC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCHMC "));
    ui->labelzCHMCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCHMCSyncStatus->setVisible(fShow);
}
