...
#include "ui_zchristmascoincontroldialog.h"
...
std::list<std::string> ZchristmascoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZchristmascoinControlDialog::listMints;

ZchristmascoinControlDialog::ZchristmascoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZchristmascoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZchristmascoinControlDialog::~ZchristmascoinControlDialog()
{
    delete ui;
}

void ZchristmascoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZchristmascoinControlDialog::updateList()
{
...
void ZchristmascoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZchristmascoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZchristmascoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZchristmascoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZchristmascoinControlDialog::GetSelectedMints()
{
...
void ZchristmascoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
